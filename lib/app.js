...
app.get('/asiakirjat/:id', function (req, res) {
    ...
    db.File.findOne({_id: fileId}).exec(function (err, file) {
        if (err) {
            ...
            file.s3Url = db.S3_ROOT + file._id + '.' + file.extension;
            file.canChangeVisibility = auth.hasPermission(username, 'hallitus');
            file.canRemoveFile = auth.hasPermission(username, 'admin');

            file.possibleVisibilities = auth.getPermissions(username).map(function (vis) {
                return {
                    name: vis,
...
app.post('/asiakirjat/:id', auth.requirePermission('hallitus'), function (req, res) {
    ...
    db.File.findOne({_id: fileId}).exec(function (err, file) {
        ...
        file.save(function (err) {
            if (err) {
                log.error('Could not change visibility from ' +
                          file.visibility + ' to ' + newVisibility +
                          ' for file: ' + reqId);
...
app.get('/asiakirjat/:id/poista', auth.requirePermission('admin'), function (req, res) {
    ...
    db.File.findOne({_id: fileId}).exec(function (err, file) {
        ...
        res.render('poista-asiakirja', file);
    });
});
...
app.post('/asiakirjat/:id/poista', auth.requirePermission('admin'), function (req, res) {
    ...
    db.File.findOne({_id: fileId}).exec(function (err, file) {
        ...
        log.debug('found doc, trying to remove: ' + reqId);
