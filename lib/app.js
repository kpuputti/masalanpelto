...
app.post('/asiakirjat/uusi', requireHallitus, function (req, res) {
    ...
    var description = trim(req.body.description) || null;
    ...
    if (name && userfile) {
        ...
        db.saveAsiakirja(asiakirja, userfile, function (err) {
            if (err) {
                console.warn('could not save asiakirja: ' + err);
                res.redirect('/');
                return;
            }
